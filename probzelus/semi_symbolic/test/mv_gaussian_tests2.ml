...
let m = Mat.of_arrays [| [| 0.; 1.|] |]

let sigma = Mat.of_arrays [| [| 1. |] |]

let d2 = mv_gaussian (mat_dot (const m) rv1) (const sigma)

let obsval = Mat.of_arrays [| [| -10. |] |]
...
let d2' = gaussian (vec_get rv1' 1) (const var)

let obsval' = -10. 

let _ = observe 0. d2' obsval';;

begin match (rv1_inner.distr, rv1'_inner.distr) with 
  | (MvNormal(mu, sigma), MvNormal(mu', sigma')) ->
    begin match (eval mu, eval sigma, eval mu', eval sigma') with
    | (ExConst mu_val, ExConst sigma_val, ExConst mu'_val, ExConst sigma'_val) ->
      assert (mu_val = mu'_val);
      assert (sigma_val = sigma'_val);
    | _ -> assert false
    end
  | _ -> assert false
end;;
