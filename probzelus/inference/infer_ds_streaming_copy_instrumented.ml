...
(* type 'a random_var = RV : ('b, 'a) Ds_streaming_low_level_instrumented.ds_node -> 'a random_var *)
...
(*             | _ -> () *)
(*           end; *)
(*           let n' = Ds_streaming_low_level_instrumented.copy_node tbl n in *)
...
(*       clear dst; *)
(*       H.iter *)
(*         (fun k n -> H.add dst k (Ds_streaming_low_level_instrumented.copy_node tbl n)) *)
(*         src *)
(*  end *)


type pstate =
  { pf_state: Ds_streaming_low_level_instrumented.pstate;
    ds_graph: Gnodes.t; }
...
let rv_kind prob rv =
  ...
  Ds_streaming_low_level_instrumented.get_distr_kind n

let rv_distr prob rv =
  let n = rv_node prob rv in
  Ds_streaming_low_level_instrumented.get_distr n

let factor' (prob, s) = Ds_streaming_low_level_instrumented.factor' (prob.pf_state, s)
let factor =
  let alloc () = () in
  let reset _state = () in
...
let rec eval' : type t.
  ...
  begin fun prob e ->
    ...
    | Ervar x ->
        ...
        let v = Ds_streaming_low_level_instrumented.value n in
        e.value <- Econst v;
        v
    | Eadd (e1, e2) ->
...
(* let rec fval : type t. t expr -> t =
   ...
   begin match e.value with
   | Econst v -> v
   | Ervar (RV x) -> Ds_streaming_low_level_instrumented.fvalue x
...
let print_ins = 
  ...
  let reset _state = () in
  let copy _src _dst = () in
  let step _state (prob, _) =
    Ds_streaming_low_level_instrumented.print_ins prob.pf_state
  in
  Cnode { alloc; reset; copy; step; }
...
let assume_constant : type a.
  ...
  fun prob d ->
  let n = Ds_streaming_low_level_instrumented.assume_constant prob.pf_state d in
...
let assume_conditional : type b c.
  ...
  let n = Ds_streaming_low_level_instrumented.assume_conditional prob.pf_state par d in
  let rv = { rv_id = n.ds_graph_node_id } in
  add_random_var prob rv n;
  rv
...
type 'a node_state =
  { node_state: 'a;
    node_graph: Gnodes.t; 
    ins : Ds_streaming_low_level_instrumented.instrumentation }

let infer n (Cnode { alloc; reset; copy; step; }) =
  let alloc () =
    { node_state = alloc ();
      node_graph = Gnodes.create 11; 
      ins = Ds_streaming_low_level_instrumented.empty_ins () }
  in
  let reset state =
  ...
  let step state (pf_prob, x) =
    let prob =
      { pf_state = Ds_streaming_low_level_instrumented.mk_pstate pf_prob state.ins;
